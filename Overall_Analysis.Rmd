...
ggsave("output/benchmark_metrics_overall.png",dpi= 300)
...
```{r}
max_run<-runs %>%
  filter(metric == "accuracy_overall") %>%
  slice(which.max(metric_score))

min_runs<-runs %>%
  filter(metric == "accuracy_overall") %>%
  slice(which.min(metric_score))

p1<-assign_tree(docs,max_run$run_id)
p2<-assign_tree(docs,min_runs$run_id)
```
