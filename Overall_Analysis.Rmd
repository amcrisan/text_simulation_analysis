...
run_act<- runs %>%
  dplyr::select(run_id,action_type,action_impact) %>%
  distinct()
...
ggplot(run_rank,aes(y=action_type,x=avg_deviation))+
  ...
  facet_grid(action_impact~.,scales="free_y")+
  theme_bw()
```
...
write.csv(topic_doc_samp, file="./output/topics-by-text.csv",quote=FALSE,row.names=FALSE)

# - words-by-topics.csv = topic_term
topic_term_samp<- topic_term %>% 
  filter(run_id %in% as.character(sample_runs$run_id))

write.csv(topic_term_samp,file="./output/words-by-text.csv",quote=FALSE,row.names=FALSE)

# - runs_sample.csv = runs
runs_samp = runs %>% filter(run_id %in% as.character(sample_runs$run_id))
write.csv(runs_samp, file="./output/runs_sample.csv")
...
write.csv(topic_count, file="./output/topic-by-count.csv",quote=FALSE,row.names=FALSE)
...
assign_tree<-function(dat = NULL,run_info=NULL){
  ...
  p<-left_join(total_grp_docs, grp_prob) %>%
    ggplot(aes(area=n, fill=avg, subgroup=topic_assigned),color="white") + 
      geom_treemap()+
      scale_fill_gradient(high = "#132B43",low="#56B1F7",limits=c(0,1),name="Median Assignment Probability")+
...
```{r}
tree_together<-cowplot::plot_grid(p1,p2)
