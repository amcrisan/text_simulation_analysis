---
title: "Simulation Analysis - COVID 19"
...
#load data
runs<-read.csv(file="sample_data/runs_covid_sample_10000.csv",stringsAsFactors = FALSE)
topic_term<-read.csv(file="sample_data/pred_topics_terms_covid_sample_10000.csv",stringsAsFactors = FALSE)
topic_doc<-read.csv(file="sample_data/pred_topics_docs_covid_sample_10000.csv",stringsAsFactors = FALSE)
gt_class<-read.csv(file="sample_data/gt_topics_covid_sample_10000.csv",stringsAsFactors = FALSE)
docs<-read.csv(file="sample_data/documents_covid_sample_10000.csv",stringsAsFactors = FALSE)
...
ggsave("output/covid_gt_topic_distribution.png",dpi= 300)
```
...
metric_labs = c("Accuracy Overall","Accuracy Overall - Weighted", "Precision - F1", "Precision - FMI", "Cluster Homogenety","Cluster Completeness","Cluster Variance","Document Shilloutte")
...
runs %>%
  ...
  ggplot(aes(y=action_type,x=metric_score))+
    ...
    scale_fill_brewer(palette = "Greys",name="Action Impact")+
    theme(axis.text.x = element_text(angle=90,hjust=1,vjust=0.5),
          legend.position="bottom")

ggsave("output/covid_benchmark_metrics_overall.pdf",dpi= 300)
...
ggplot(run_rank,aes(y=action_type,x=avg_deviation))+
  ...
  facet_grid(action_impact~.,scales="free_y",space='free_y')+
  theme_bw()


ggsave("output/covid_action_impact.png",height=5,dpi= 300)
```
...
write.csv(topic_doc_samp, file="./visualizations/data/covid/topics-by-text.csv",quote=FALSE,row.names=FALSE)

# - words-by-topics.csv = topic_term
topic_term_samp<- topic_term %>% 
  filter(run_id %in% as.character(sample_runs$run_id))

write.csv(topic_term_samp,file="./visualizations/data/covid/words-by-topics.csv",quote=FALSE,row.names=FALSE)

# - runs_sample.csv = runs
runs_samp = runs %>% filter(run_id %in% as.character(sample_runs$run_id))
write.csv(runs_samp, file="./visualizations/data/covid/runs_sample.csv",row.names=FALSE,quote=TRUE)
...
write.csv(topic_count, file="./visualizations/data/updated/topics-by-count.csv",quote=FALSE,row.names=FALSE)
...
save_plot("./output/covid_treemap_comparison.pdf", tree_together, base_height=5,base_asp=1.1, ncol = 2,dpi=300)
```
